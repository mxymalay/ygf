...
class SwitchSettingsWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        weight_tab_title = QLabel(u"按重量分流：官方/私域累计重量（kg）")
        weight_tab_title.setStyleSheet("font-size: 18px; font-weight: 900; color: #38BDF8; margin-top: 12px;")
        weight_tab_tip = QLabel(u"重量折线只使用稳定称重分流记录；切换到金额分流模式的时段会留白，不会把金额当作重量显示。")
        weight_tab_tip.setWordWrap(True)
        weight_tab_tip.setStyleSheet("font-size: 12px; color: #94A3B8; font-weight: normal;")
        weight_tab_layout.addWidget(weight_tab_title)
        weight_tab_layout.addWidget(weight_tab_tip)
