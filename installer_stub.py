...
try:
    import queue
except ImportError:  # pragma: no cover - Python 2 is not supported, kept for Win7 tooling
...
def _emit_progress(progress, message, current=None, total=None):
    """Send an installer status event without coupling workers to Tk."""
    if not callable(progress):
        return
    try:
        progress(str(message or ""), current, total)
    except Exception:
        # Progress is best-effort; an old Tk/Win7 display must never abort a
        # valid installation.
        pass
...
def _safe_extract_payload(target_dir, progress=None):
    ...
    with zipfile.ZipFile(payload, "r") as archive:
        ...
        file_entries = [info for info in entries if not info.is_dir()]
        ...
        for info in entries:
            ...
            _emit_progress(
                progress,
                "正在释放：%s" % info.filename.replace("/", os.sep),
                extracted_files,
                total_files,
            )
...
def _install(target_dir, display_name, icon_preset="yangguofu", progress=None):
    ...
    _emit_progress(progress, "正在释放程序文件…", 0, 0)
    _safe_extract_payload(target_dir, progress=progress)
    _emit_progress(progress, "正在写入安装配置…")
    ...
    _emit_progress(progress, "正在创建快捷方式和卸载入口…")
    ...
    _emit_progress(progress, "安装文件已就绪，正在完成收尾…", 1, 1)
...
def main():
    ...
    def show_progress_dialog(target, display_name, icon_preset):
        """Run the blocking installer on a worker and keep Tk repainting.

        Tk itself stays on the main thread (required by Win7's Tcl/Tk build);
        only file/service work runs in the worker.  A queue avoids touching
        widgets from the worker and also keeps the old no-Tk fallback intact.
        """
        progress_dialog = tk.Toplevel(root)
        progress_dialog.title("正在安装/更新 YGF POS")
        progress_dialog.geometry("680x430")
        progress_dialog.minsize(680, 430)
        progress_dialog.resizable(False, False)
        progress_dialog.transient(root)
        progress_dialog.protocol(
            "WM_DELETE_WINDOW",
            lambda: detail_var.set("安装正在进行，请等待当前步骤完成；为避免留下半套文件，暂不能关闭。"),
        )
        try:
        ...
        outer = ttk.Frame(progress_dialog, padding=22)
        outer.pack(fill="both", expand=True)
        ttk.Label(outer, text="正在安装/更新", font=("Microsoft YaHei", 18, "bold")).pack(anchor="w")
        ttk.Label(
            outer,
            text="安装器正在停止旧服务、释放程序文件并创建快捷方式。请勿关闭电脑。",
            wraplength=620,
        ).pack(anchor="w", pady=(6, 14))
        detail_var = tk.StringVar(value="准备开始…")
        ttk.Label(outer, textvariable=detail_var, wraplength=620).pack(anchor="w", pady=(0, 8))
        progress_bar = ttk.Progressbar(outer, orient="horizontal", mode="indeterminate", length=620)
        ...
        scrollbar = ttk.Scrollbar(log_frame, orient="vertical", command=detail.yview)
        detail.configure(yscrollcommand=scrollbar.set, state="disabled")
        ...
        def install_worker():
            try:
                _install(target, display_name, icon_preset, progress=progress_callback)
                events.put(("done", "安装步骤已完成。", None, None))
            except Exception as exc:
                events.put(("error", str(exc) or repr(exc), None, None))

        def append_detail(message):
            detail.configure(state="normal")
            detail.insert("end", str(message or "") + "\n")
            detail.see("end")
        ...
        def poll_events():
            try:
                while True:
                    ...
                    kind, message, current, total = event
                    if kind == "progress":
                        detail_var.set(str(message or ""))
                        append_detail(message)
                        if total and int(total) > 0:
                            progress_bar.stop()
                            progress_bar.configure(mode="determinate", maximum=int(total), value=int(current or 0))
                    elif kind == "done":
                        progress_bar.stop()
                        progress_bar.configure(mode="determinate", maximum=1, value=1)
                        append_detail(message)
                        finish_dialog()
                        ...
                        icon_box.configure(state="readonly")
                        status.configure(text="安装失败，可检查详情后重试。")
                        messagebox.showerror("安装失败", str(message), parent=root)
                        return
            except queue.Empty:
                pass
            try:
        ...
        icon_box.configure(state="disabled")
        progress_dialog.after(80, poll_events)
        threading.Thread(target=install_worker, name="YGFInstallerWorker", daemon=True).start()
    ...
    def install_click():
        ...
        try:
            ...
            status.configure(text="正在安装，请查看安装详情窗口…")
            show_progress_dialog(target, display_name, icon_preset)
        except Exception as exc:
            messagebox.showerror("安装失败", str(exc), parent=root)

    install_button = ttk.Button(buttons, text="安装 / 更新", command=install_click)
    install_button.pack(side="left", ipadx=24, ipady=10)
    if existing:
