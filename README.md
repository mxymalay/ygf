...
3. 编译完成后，项目 `dist/` 目录生成一个带时间戳的 `setup_YYYYMMDD_HHMMSS.exe` 安装包；上一版会自动保存在 `dist/backup/`，且备份目录只保留一版。
4. **推荐使用**：双击 `dist/setup_YYYYMMDD_HHMMSS.exe`，选择安装目录。安装包会在安装目录释放 `启动.exe`、桥接服务、维修工具，并创建快捷方式和“卸载.exe”。
