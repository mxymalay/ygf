...
## 重要：当前 VSPD 方案与新服务不能同时占用 COM2/COM3

现场当前的 VSPD 分流能正常工作，ScaleBridge 不会自行更换或卸载它。若要迁移，必须在收银停用窗口内由技术人员明确操作：停止 VSPD 分流、确认 `COM2` 和 `COM3` 已释放、建立 com0com 配对、再启动服务。回退时先停服务，再恢复原 VSPD 配置。
...
2. 在 com0com 的 Setup Command Prompt 中创建并核对配对。例如：
   ...
   然后使用 `list` 核对另一端确实是 `CNCB0`、`CNCB1`。`COM10 ↔ COM11` 支付对必须按店内既有支付插件的要求建立，且不可覆盖已被收钱吧占用的 `COM10`。
...
- 移除服务前先停止服务；不要删除或改写不属于本方案的 VSPD 文件。
